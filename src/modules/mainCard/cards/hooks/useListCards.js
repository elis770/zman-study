...
export const useListCards = () => {
  ...
  const zmanimCards = Object.entries(zmanim || {})
    ...
    .map(([key, value]) => {
      ...
      const isVisible = visibleZmanim?.[key] || (legacyKeyMap[key] && visibleZmanim?.[legacyKeyMap[key]]);
  ...
  const studyCards = (study?.studyCards || [])
    .filter(item => {
      const key = item.key;
      const legacyKey = key.toLowerCase().replace('_', '-');
      return visibleEstudios?.[key] || visibleEstudios?.[legacyKey];
    })
    .map(item => {
